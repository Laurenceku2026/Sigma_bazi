...
def sync_app_user(email: str | None = None) -> dict | None:
    ...
    try:
        ...
        st.error(f'无权访问本应用：{e}')
        return None
    except Exception as e:
        st.warning(f'用户同步暂不可用（不影响本地排盘）：{e}')
        return None
...
with tab1:
    ...
    if generate_btn and name:
        ...
        with st.spinner("🔮 正在排盘与生成报告..."):
            ...
            # 先在本 App schema 登记用户（含 email），再写报告，防止跨 App 污染
            if supabase_client:
            ...
            if selected_tier != 'free' and report_gen:
                try:
                    ...
                    # 保存到本 App schema（自动附带 app_id 并校验用户归属）
                    if supabase_client:
                        try:
